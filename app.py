        ...
        if st.button("🚨 実際の着順・ハナ争いと照合して復習"):
            if not api_key or not result_copypaste.strip():
                ...
                try:
                    ...
                    # ⭕ 【究極修正】エラーの原因だったf-stringを廃止し、安全な通常トリプルクォート文字列に変更しました。
                    review_prompt_template = """あなたは総監督Baruの右腕AIだ。提示された【当時の予想指示書】と、実際の【レース結果コピペ】を徹底的に突き合わせ、超精密な反省・復習レポートを作成せよ。

【超重要：タイトル認識の掟】
最上部には、必ず総監督が指定したタイトル「{TITLE}」を引用して「### 🏁 {TITLE} 答え合わせ・戦果照合」という見出しからスタートせよ。
