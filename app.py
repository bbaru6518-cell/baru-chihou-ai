...
def parse_netkeiba_complete(raw):
    ...
    horses = []
    if not raw or not raw.strip(): return {"race_info": info, "horses": horses}
    try:
        m = re.search(r'(\d+)R\s+([^\n]+)', raw)
        if m: info["race_num"], info["race_name"] = int(m.group(1)), m.group(2).strip()
        tm = re.search(r'([ダ芝])(\d+)m', raw)
        if tm: info["track_type"], info["distance"] = tm.group(1), int(tm.group(2))
    except: pass

    blocks = re.split(r'(?:\n|^)(\d+)\s+(\d+)\s*\n?--', raw)[1:]
    for i in range(0, len(blocks) - 2, 3):
        try:
            wk, ub, body = int(blocks[i]), int(blocks[i+1]), blocks[i+2].strip()
            ...
            for idx, s_pos in enumerate(starts):
                try:
                    ...
                    meta, p_name = r_lns[0].split(), r_lns[1]
