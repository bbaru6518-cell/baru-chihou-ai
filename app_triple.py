            ...
            if not api_key or not result_copypaste.strip():
                ...
                try:
                    ...
                    with st.spinner("トリプル馬単地方競馬の1・2着展開のズレから猛省・復習中..."):
                        ...
                        st.success(f"💾 ファイル名を「{new_filename}」に変更し、トリプル地方復習ログを完全保存しました！")
                        st.rerun()
                except Exception as e:
                    st.error(f"復習解析エラー: {e}")
    else:
        st.info("まだ保存されたトリプル馬単地方競馬の予想ログはありません。")
...
with col1:
    st.subheader("📋 トリプル対象地方レース 馬柱・オッズ混在テキスト入力")
    url_input = st.text_input("🔗 地方レースURL（netkeiba等）")
    manual_data = st.text_area("✍️ トリプル馬単地方競馬対象レースコピペデータ", height=500)
    
    if st.button("🚀 トリプル馬単地方競馬・構造解剖＆勝負指示書生成"):
        try:
            target_data = ""
            if url_input:
            ...
            if not api_key or not target_data:
                ...
                # 【トリプル馬単地方競馬・高速特化型プロンプト】馬単の1着2着を執念で当てるためのロジック
                base_instruction = """あなたはトリプル馬単地方競馬を完全ハックするプロ競馬AIであり、総監督Baruの絶対的右腕だ。
...
2着：〇, 〇, 〇, 〇, 〇, 〇, 〇"""
                ...
                with st.spinner(f"🚀 トリプル馬単地方競馬・連対圏（1,2着）をマッピング中... ({m_name})"):
                    ...
                    st.toast("💾 トリプル馬単地方競馬予想ログを保存しました！", icon="💾")
                    st.rerun()
        except Exception as e:
            st.error(f"解析エラー: {e}")

with col2:
    st.subheader("📊 トリプル馬単地方競馬指示書 ＆ 復習ルーム連動表示")
    if st.session_state["res"]:
        st.markdown(st.session_state["res"])

# ⭕ 【残党完全消去】最下部のフッタークレジットも完全置換！
st.caption("Baru Stable AI Triple Local Pro v24.8.5 - Fast Edition")
